...
for i, nu in enumerate(nu_try):
    test = full_shell(project = proj_name+str(idx_try), simpProps = geo_prop_four, imperfection = 0.05)
    # test.mesh_shape = 'tri'
    test.mesh_order = 'quadratic'
    test.nu_shell = nu
    jname_lin = test.run_linear_model()
    jname_multi = test.make_nonlin_multi_buckle(bdamp, max_temp_mult = 0.35, num_steps = 30)
