...
def postBuck(project,H,R,t1,t2,theta,w,E1,E2,E_cap,h_element,bdamp,imper,eig_idx,ncores,run_sim=False):
    ''' 
    **Create Abaqus model, run FEA, and output results file
    ...
    mat1.Damping(alpha=0., beta=bdamp)
    ...
    mat2.Hyperelastic(materialType=ISOTROPIC, table=((nh_C10_2,nh_D1_2), ), testData=OFF,
        type=NEO_HOOKE, volumetricResponse=VOLUMETRIC_DATA)
    mat2.Damping(alpha=0., beta=bdamp)
    ...
    mat_cap.Hyperelastic(materialType=ISOTROPIC, table=((nh_C10_cap,nh_D1_cap), ), testData=OFF,
        type=NEO_HOOKE, volumetricResponse=VOLUMETRIC_DATA)
    mat_cap.Damping(alpha=0., beta=bdamp)
    ...
    p4.setElementType(elemTypes=(ElemType(elemCode=S4R, elemLibrary=STANDARD, secondOrderAccuracy=OFF, 
        hourglassControl=DEFAULT), ElemType(elemCode=S3, elemLibrary=STANDARD)), regions=(p3.faces, ))
    p4.generateMesh()
    ...
    m.ImplicitDynamicsStep(alpha=DEFAULT, amplitude=RAMP, 
       application=QUASI_STATIC, initialConditions=OFF, initialInc=0.001, maxInc=
        0.05, maxNumInc=nincre, minInc=1e-09, name='Step-1', nlgeom=ON, nohaf=OFF, 
       previous='Initial')

    '''REQUESTS'''
    #m.HistoryOutputRequest(createStepName='Step-1', name='H-Output-1', rebar=EXCLUDE, region=MODEL, sectionPoints=DEFAULT, variables=('ALLSE', ));
    #m.HistoryOutputRequest(createStepName='Step-1', name='H-Output-2', rebar=EXCLUDE, region=a.sets['rp2'], sectionPoints=DEFAULT, variables=('CVOL', 'PCAV'));
    ...
    m.HistoryOutputRequest(createStepName='Step-1', name='H-Output-2', numIntervals=nincre_output, rebar=EXCLUDE, 
        region=set_rp2, sectionPoints=DEFAULT, timeMarks=OFF, variables=('PCAV', 'CVOL'))
    ...
    '''BCS'''
    m.VelocityBC(amplitude=UNSET, createStepName='Initial',distributionType=UNIFORM, fieldName='', localCsys=None, name='BC-2', 
        region=a.sets['Merged-1.ring'], v1=0.0, v2=0.0, v3=0.0, vr1=0.0, vr2=0.0, vr3=0.0)

    '''TEMPERATURE FIELDS FOR VOLUME CHANGE'''
    temp_set = -0.2 #originally -0.332
    ...
    m.Temperature(amplitude='Amp-1', createStepName='Step-1', 
        crossSectionDistribution=CONSTANT_THROUGH_THICKNESS, distributionType=
        UNIFORM, magnitudes=(temp_set, ), name='Predefined Field-2', region=a.sets['rp2'])
    ...
    multiprocessingMode=DEFAULT, name=project+'_post_buckling', nodalOutputPrecision=SINGLE, 
    numCpus=ncores, numDomains=ncores, numGPUs=0, queue=None, resultsFormat=ODB, scratch=
    '', type=ANALYSIS, userSubroutine='', waitHours=0, waitMinutes=0)
    mdb.saveAs(project+'_post_buckling')
    if run_sim:
        j.submit()
        j.waitForCompletion()
    f1=open(project+'_post_buckling_parameters.txt','w')
    f1.write('H, R, t1, t2, theta, w, E1, E2, E_cap, h_ele, bdamp, imper, eig_idx, ncores\n')
    f1.write(str(H)+' '+str(R)+' '+str(t1)+' '+str(t2)+' '+str(theta)+' '+str(w)+' '+str(E1)+' '+str(E2)+' '+str(E_cap)+' '+str(h_element)+' '+str(bdamp)+str(imper)+' '+str(eig_idx)+' '+str(ncores))
    f1.close()
    #shutil.move(olddir+'/'+project+'_nonlin_buckling_parameters.txt', futuredir+'/'+project+'_nonlin_buckling_parameters.txt')
    #os.chdir(futuredir)
    return 